# アプリ名
クイズゲーム

# 概要

youtubeの解説動画を参考にDBを使わずjavascriptとhtmlだけで作った

選択式のクイズゲームです

回答のボタンをクリックすると正解/不正解が表示され、

最後の問題が終了した後に合計正解数が表示されます(現在5問)

トップページを作成し、問題終了後にトップページへ遷移し、クイズを最初から繰り返せるよう追記しました

# 本番環境
https://tech-student-oono.github.io/quizgame/

# 制作背景(意図)

私はプログラミングを勉強している中で「学習を繰り返すことで定着率を上げる」ということを知りました、

その中で「手軽に学習を繰り返すことができる物ができないか？」と考えこのクイズゲームを制作することを思いつきました。

さらに学習の幅を広げるためには問題数を増やすことも必要であるため、コードの簡単な編集で問題が拡張できるjavascriptを使いました。

これにより、問題を作る際には「学習に役立つ問題」を意識し、正解/不正解の際には「なぜその答えに行き着いたのか？」をその都度意識することで

「繰り返しの学習」が手軽に実現できると考えました、それがこのアプリを制作したいと思った理由です。


# DEMO
問題と選択肢が表示されます、単語をクリックすることで回答できます。
<img width="1440" alt="スクリーンショット 2020-10-14 17 51 21" src="https://user-images.githubusercontent.com/68591022/95966357-fe7bf180-0e45-11eb-84fd-7a3258a86c2d.png">

クリックの後、正解/不正解が表示され次の問題(ページ)に進みます。
<img width="1440" alt="スクリーンショット 2020-10-14 17 51 29" src="https://user-images.githubusercontent.com/68591022/95966362-0176e200-0e46-11eb-8f54-6db667a03ed1.png">

全ての問題を回答し終わると正解の合計数が表示されます。
<img width="1440" alt="スクリーンショット 2020-10-14 17 51 44" src="https://user-images.githubusercontent.com/68591022/95966368-0340a580-0e46-11eb-9a0b-a7ec2f385ec3.png">

# 使用技術(開発環境)
## バックエンド
Ruby, Ruby on Rails
## フロントエンド
HTML, Javascript

# 課題や今後実装したい機能
問題数を増やすこととユーザーが自分で問題を作成できる機能を実装したいと考えております。
